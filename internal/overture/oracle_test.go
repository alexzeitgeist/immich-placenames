...
// leaves the larger association. Stuttgart must remain eligible as a county.
func TestOracleGermanDistrictNames(t *testing.T) {
	...
	places := []struct {
		...
		{"jestetten", geocode.Point{Lat: 47.6467, Lon: 8.5686}, "Baden-Württemberg"},
		{"litzelstetten", geocode.Point{Lat: 47.7069, Lon: 9.1667}, "Baden-Württemberg"},
		{"bersenbrück", geocode.Point{Lat: 52.5553, Lon: 7.9439}, "Lower Saxony"},
		{"stuttgart", geocode.Point{Lat: 48.7758, Lon: 9.1829}, "Baden-Württemberg"},
	}
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for i, place := range places {
				res, err := p.Resolve(context.Background(), place.pt)
				if err != nil {
					t.Fatal(err)
				}
				if res.City != tc.cities[i] || res.State != place.state {
					t.Errorf("%s: city %q in %q, want %q in %q", place.name, res.City, res.State, tc.cities[i], place.state)
				}
			}
		})
	}
